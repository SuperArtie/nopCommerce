...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CategoryController : BaseAdminController
    {
        ...
        [HttpPost]
        public virtual IActionResult SearchCategoryName(string searchTerm)
        {
            ...
            {
                var categoryNames = searchTerm.Split(new[] { ">>" }, StringSplitOptions.None).ToList();
                categoryName = categoryNames[categoryNames.Count - 1].Trim();
                parentCategoryName = categoryNames[categoryNames.Count - 2].Trim();
            }
            ...
            if (!string.IsNullOrWhiteSpace(categoryName) && categoryName.Length >= 3)
                categoryItems.AddRange(_categoryService.GetAllCategories(categoryName, showHidden: true).Select(x => new SelectListItem
                {
                    Text = _categoryService.GetFormattedBreadCrumb(x), Value = x.Id.ToString()
                }).ToList());
            else
            {
                var categoryRepo = EngineContext.Current.Resolve<IRepository<Category>>();

                if (!categoryRepo.Table.Any(x => x.Name == parentCategoryName))
                    return Json(categoryItems);

                var parentCategoryId = categoryRepo.Table.Where(x => x.Name == parentCategoryName)
                    .Select(x => x.Id).First();

                if (string.IsNullOrWhiteSpace(categoryName))
                    categoryItems.AddRange(_categoryService.GetAllCategoriesByParentCategoryId(parentCategoryId, true)
                        .Select(x => new SelectListItem
                        {
                            Text = _categoryService.GetFormattedBreadCrumb(x),
                            Value = x.Id.ToString()
                        }));
                
                else
                    categoryItems.AddRange(categoryRepo.Table
                        .Where(x => x.ParentCategoryId == parentCategoryId && x.Name.StartsWith(categoryName)).ToList()
                        .Select(x => new SelectListItem
                        {
                            Text = _categoryService.GetFormattedBreadCrumb(x),
                            Value = x.Id.ToString()
